...
export default function Testimonials() {
  ...
  useEffect(() => {
    ...
    if (!wrapper || !spacer || !section) return;
    ...
    function calcProgress(): number {
      ...
      const triggerStart = vh * 0.1;
      const triggerEnd = spacerRect.height - vh * 0.1;
      const current = -spacerRect.top + triggerStart;
      return Math.min(1, Math.max(0, current / triggerEnd));
    }
    ...
    if (totalImgs > 0) {
      imgs.forEach(img => {
        if (img.complete) { loadedCount++; if (loadedCount === totalImgs) onScroll(); }
        else img.addEventListener("load", () => { loadedCount++; if (loadedCount === totalImgs) onScroll(); }, { once: true });
